...
class AccountMove(models.Model):
    ...
    def _xml(self):
        ...
        ET.SubElement(DireccionEmisor, 'dte:Direccion').text = '%s %s' % (
        self.company_id.street, self.company_id.street2 or '')
        ET.SubElement(DireccionEmisor, 'dte:CodigoPostal').text = self.company_id.zip or '0'
        ET.SubElement(DireccionEmisor, 'dte:Municipio').text = self.company_id.city or ''
        ET.SubElement(DireccionEmisor, 'dte:Departamento').text = self.company_id.state_id.name or ''
        ET.SubElement(DireccionEmisor, 'dte:Pais').text = self.company_id.country_id.code or ''
        datosEmisor = {'CorreoReceptor': self.partner_id.email or self.company_id.fe_other_email or '',
                    ...
                    if not self.env["account.tax.group"]._apply_withholding(tax['tax_group_id']):
                        ET.SubElement(
                            ...
                            'dte:TotalImpuesto',
                            {
                                ...
                                'TotalMontoImpuesto': str(round(tax["tax_group_amount"], 2))
                            }
                        )
        if self.fe_type == 'FESP':

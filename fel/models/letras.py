...
# Para definir la moneda me estoy basando en los codigo que establece el ISO 4217
# Decido? poner las variables en ingles, porque es mas sencillo de ubicarlas sin importar el pais
# Si, ya se que Europa no es un pais, pero no se me ocurrio?un nombre mejor para la clave.
...
def to_word(number, mi_moneda=None):
    ...
    Examples:
        ...
        'Mil Cuatrocientos Ochenta Y Un Euros con Un CYù?sntimo'
    """
    if mi_moneda != None:
        try:
            ...
            return "Tipo de moneda invo^yu'lida"
    else:
        entero = ""
        fraccion = ""
    ...
    num_decimals ='{:,.2f}'.format(round(number,2)).split('.') #So^®lo se aceptan 2 decimales
    ...
    if num_decimals != ['00']:
        decimales = str(num_decimals).replace('[','')
        decimales = str(decimales).replace(']','')
        decimales = str(decimales).replace("'",'')
        sentence = sentence.upper() + ' QUETZALES CON ' + decimales + '/100' #' '.join(human_readable_decimals).replace('  ',' ').title().strip() + ' Centavos'
    else:
        sentence = sentence.upper() + ' QUETZALES CON ' +'00/100'
    return sentence
